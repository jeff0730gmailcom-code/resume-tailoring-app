...
format each step is timed against):
  ...
  9. Response            - /download (zip named Name_stack_company)
...
async def _render_and_cache_pdf(file_id: str, perf: PerfReport):
    ...
    with perf.stage("PDF Generation"):
        # Private uploads render from their own Jinja file (a per-upload copy
        # of the matched layout: Quang→quang, Dejan→dejan, …). Built-ins use
        # named gallery layouts.
...
async def download_resume(
    ...
    Returns a zip named `{Name}_{stack}_{company}.zip` containing
    ...
    zip_path = file_utils.write_download_zip(file_id, zip_stem, inner_name, source_path, format)
